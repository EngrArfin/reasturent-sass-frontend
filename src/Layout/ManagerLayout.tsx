...
const ManagerLayout = () => {
    ...
    return (
        <div className="flex h-screen overflow-hidden bg-gradient-to-r from-[#052318] via-[#0A1C19] to-[#0F131B]">
            ...
            {/* Main Content */}
            <div
                className={`flex flex-col flex-1 transition-all duration-200 ease-in-out ${!shouldHideSidebar() ? "lg:ml-72" : ""
                    }`}
            >
                {/* Navbar */}
                {!shouldHideNavbar && (
                ...
                <main
                    className={`flex-1 overflow-y-auto mt-16 text-white bg-[#121826] ${isSidebarOpen ? "pt-4 md:pt-10" : "p-4 md:p-10"
                        }`}
                >
                    <Outlet />
                </main>
